...
public class LOSEntity : MonoBehaviour {
    ...
    void Start()
    {
		if (gameObject.GetComponent<RTSObject> () != null) {
			RTSObject obj = (RTSObject)this.gameObject.GetComponent<RTSObject>();
			if (obj.owner != null){
				if (obj.owner.username != null){
					string team = obj.owner.username;
					Debug.Log (team);
